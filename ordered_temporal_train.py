...
from model.data_temporal import ImageFolder
...
solver = VGGV().cuda()
solver.load_state_dict(torch.load('./weights/temporal_model.th'))
tic = time()
no_optim = 0
total_epoch = 100
train_epoch_best_loss = 0.5795
LEARNING_RATE = 8e-09
...
#\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
alpha = 0
for epoch in range(35, total_epoch + 1):
    ...
    correct = 0
    if epoch<10:
        savepath = './logs/loss_temporal/loss_epoch_0'+str(epoch)+'.txt'
    else:
        savepath = './logs/loss_temporal/loss_epoch_'+str(epoch)+'.txt'
    for videoseg, mask in data_loader_iter:
        ...
        optimizer.zero_grad()
        #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        outputs1 = solver.forward(videoseg)
        if torch.max(outputs1.data, 1).indices==mask:
            correct += 1
        loss1 = cost(outputs1, mask)
    ...
    if no_optim > 3:
        ...
        old_lr = update_lr(old_lr,5.0, factor = True, mylog = mylog)
    mylog.flush()
