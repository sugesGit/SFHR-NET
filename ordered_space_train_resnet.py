...
for epoch in range(1, total_epoch + 1):
    ...
    correct = 0
    if epoch<10:
        savepath = './logs/loss_resnet/loss_epoch_0'+str(epoch)+'.txt'
    else:
        savepath = './logs/loss_resnet/loss_epoch_'+str(epoch)+'.txt'
    for videoseg, mask in data_loader_iter:
        ...
        print('alpha, loss, loss1, loss2:', alpha, loss, loss1, loss2)
    ...
    if train_epoch_loss >= train_epoch_best_loss:
        ...
        torch.save(solver.state_dict(), 'weights/weight_resnet/'+NAME+'.th')
    if no_optim > 6:
        print ('early stop at %d epoch' % epoch,file = mylog)
        print ('early stop at %d epoch' % epoch)
        break
    torch.save(solver.state_dict(), 'weights/weight_resnet/'+NAME+str(epoch)+'.th')
    if no_optim > 3:
        if old_lr < 5e-9:
            break
        no_optim = 0
        solver.load_state_dict(torch.load('weights/weight_resnet/'+NAME+'.th'))
